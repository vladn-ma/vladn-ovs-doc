/*
 * $Id: intr.c,v 1.65 2012/07/16 11:48:27 assaf Exp $
...
soc_intr_arl_error(int unit, uint32 ignored)
{
    ...
    /* FIXME: bucket overflow? need to set ARL_CONTROL.CLEAR_LEARN_TRAP? */
    /* FIXME: parity error?  */
    soc_cm_debug(DK_ERR,
...
int 
soc_get_interrupt_id(int unit, soc_reg_t reg, int reg_index, soc_field_t field, int* interrupt_id)
{
    soc_interrupt_db_t *interrupts_arr;
    int nof_interrupts;
    int i;
...
soc_get_interrupt_id_specific(int unit, int reg_adress, int reg_block, int field_bit, int* interrupt_id)
{
    ...
    int nof_interrupts;
    soc_field_info_t *finfop;
    int block, port=0, is_port_block=0;
    uint8 acc_type;
    int i;
    ...
    *interrupt_id = -1;
    ...
    interrupts_arr = SOC_CONTROL(unit)->interrupts_info->interrupt_db_info;
    /* find the block index - port parameter*/
    
        for(i = 0 ; SOC_BLOCK_INFO(unit, i).type >= 0; ++i ) { 
            if (SOC_INFO(unit).block_valid[i]) {
                if (reg_block == SOC_BLOCK_INFO(unit,i).cmic) {
                    if(reg_block != SOC_CMIC_BLK_CLP_0_INDX && 
                        reg_block != SOC_CMIC_BLK_CLP_1_INDX && 
                        reg_block != SOC_CMIC_BLK_XLP_0_INDX && 
                        reg_block != SOC_CMIC_BLK_XLP_1_INDX) {
                    reg_block -= SOC_BLOCK_NUMBER(unit, i);
                    break;
                     } else {
                        port = REG_PORT_ANY/*SOC_BLOCK_PORT(unit, i)*/;
                        is_port_block = 1;
                        break;
                     }
                }
    ...
    if(port < 0 && port != REG_PORT_ANY) {
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM, (_SOC_MSG("block not valid.\n")));
    }
    ...
    for(i=0 ; i < nof_interrupts; i++) {
       ...
       if(reg_adress ==  soc_reg_addr_get(unit, interrupts_arr[i].reg, port, interrupts_arr[i].reg_index, &block, &acc_type)) {

            if(is_port_block && 
                (block == SOC_CMIC_BLK_CLP_0_INDX || block == SOC_CMIC_BLK_CLP_1_INDX  || block == SOC_CMIC_BLK_XLP_0_INDX || block == SOC_CMIC_BLK_XLP_1_INDX )) {
                  *interrupt_id = i;
                   break;              
            }
...
int soc_interrupt_get_block_index_from_port(int unit, int interrupt_id, int port)
{
    ...
    if(SOC_IS_ARAD(unit) &&  (SOC_BLOCK_IN_LIST(block_types, SOC_BLK_CLP) || SOC_BLOCK_IN_LIST(block_types, SOC_BLK_XLP) )) {
        index = 
            port == SOC_BLOCK_PORT(unit, CLP_BLOCK(unit, 0)) ? 0 : 
            port == SOC_BLOCK_PORT(unit, CLP_BLOCK(unit, 1)) ? 1 : 
            port == SOC_BLOCK_PORT(unit, XLP_BLOCK(unit, 0)) ? 2 : 
            port == SOC_BLOCK_PORT(unit, XLP_BLOCK(unit, 1)) ? 3 : 
            -1 ;
        return index;            
...
int soc_interrupt_update_storm_detection(int unit, int block_instance, soc_interrupt_db_t *inter) 
{
    ...
    if (inter->storm_timed_count[inf_index] > 0 && inter->storm_timed_period[inf_index] > 0) {

        if ((current_time - inter->storm_detection_start_time[inf_index]) > inter->storm_timed_period[inf_index]) {
...
int soc_interrupt_is_storm(int unit, int block_instance, soc_interrupt_db_t *inter, int *is_storm_count_period,int *is_storm_nominal) {
    ...
    *is_storm_nominal  = 0x0;
    ...
    if ((inter->storm_timed_count[inf_index] != 0x0) && (inter->storm_detection_occurrences[inf_index] >= inter->storm_timed_count[inf_index])) {
...
int soc_interrupt_clear_all(int unit){
    ...
    for (bi_index = 0;  SOC_BLOCK_INFO(unit, bi_index).type >= 0; bi_index++ ) {
        for (inter = 0; inter < nof_interrupts ; inter++) {
            ...
            rc = soc_interrupt_get(unit, bi->number, &(interrupts_arr[inter]), &is_on );

            if (is_on) {
                if(NULL != interrupts_arr[inter].interrupt_clear) {
                    rc = interrupts_arr[inter].interrupt_clear(unit, bi->number , inter);
                    _SOC_IF_ERR_EXIT(rc);
                }
            }
        }
...
#ifdef BCM_CMICM_SUPPORT
        low_mask = soc_pci_read(unit, CMIC_CMCx_PCIE_IRQ_MASK3_OFFSET(SOC_PCI_CMC(unit)));
        high_mask = soc_pci_read(unit, CMIC_CMCx_PCIE_IRQ_MASK4_OFFSET(SOC_PCI_CMC(unit)));
#endif
    }
...
int soc_sort_interrupts_according_to_priority(int unit, soc_interrupt_cause_t* interrupts, uint32 interrupts_size)
{
    ...
    int  array_left_index[SOC_INTERRUPT_DB_PRIORITY_MECHANISM_MAX_LEVELS] ;
    int  array_right_index[SOC_INTERRUPT_DB_PRIORITY_MECHANISM_MAX_LEVELS];
    int  index = 0, left_index, right_index, swap;
    soc_interrupt_cause_t pivot;
    int left_interrupt_priority,right_interrupt_priority, pivot_priority;
    soc_interrupt_db_t* interrupts_db;
    int stop_check_flag;
    ...
    interrupts_db = SOC_CONTROL(unit)->interrupts_info->interrupt_db_info;
    if (NULL == interrupts_db ) {
        _SOC_EXIT_WITH_ERR(SOC_E_UNAVAIL, (_SOC_MSG("Interrupt DB does not exist "))); 
    }

    array_left_index[0] = 0; array_right_index[0]=interrupts_size;
    while (index >= 0) {
        left_index = array_left_index[index]; right_index = array_right_index[index] -1;
        if (left_index  < right_index) {
            pivot= interrupts[left_index];
            pivot_priority = ((((interrupts_db[pivot.id]).flags)& SOC_INTERRUPT_DB_FLAGS_PRIORITY_MASK) >> SOC_INTERRUPT_DB_FLAGS_PRIORITY_BITS_LSB);
            while (left_index < right_index) {
                stop_check_flag = 0;
                while(!stop_check_flag) {
                    right_interrupt_priority = (((interrupts_db[interrupts[right_index].id]).flags & SOC_INTERRUPT_DB_FLAGS_PRIORITY_MASK) >> SOC_INTERRUPT_DB_FLAGS_PRIORITY_BITS_LSB);
                    if((right_interrupt_priority>=pivot_priority)&&(left_index < right_index)) {
                        right_index--;
                    }
                    else {
                        stop_check_flag = 1;
                ...
                if (left_index < right_index) {
                   interrupts[left_index++] = interrupts[right_index];
                }
                stop_check_flag = 0;
                while (!stop_check_flag) {
                   left_interrupt_priority = (((interrupts_db[interrupts[left_index].id]).flags & SOC_INTERRUPT_DB_FLAGS_PRIORITY_MASK) >> SOC_INTERRUPT_DB_FLAGS_PRIORITY_BITS_LSB);
                   if((left_interrupt_priority<=pivot_priority) && (left_index < right_index)) {
                       left_index++;
                   }
                   else {
                       stop_check_flag = 1;
                ...
                if (left_index < right_index) {
                   interrupts[right_index--] = interrupts[left_index];
                }
           }
           
           interrupts[left_index] = pivot; 
           array_left_index[index + 1] = left_index + 1; 
           array_right_index[index + 1] = array_right_index[index]; 
           array_right_index[index++] = left_index;
           if ((array_right_index[index]-array_left_index[index]) > (array_right_index[index - 1]-array_left_index[index - 1])) {
               swap = array_left_index[index]; 
               array_left_index[index] = array_left_index[index -1]; 
               array_left_index[index - 1] = swap;
               swap=array_right_index[index];
               array_right_index[index] = array_right_index[index - 1]; 
               array_right_index[index - 1] = swap; 
           }
        } else {
            index--; 
          }
